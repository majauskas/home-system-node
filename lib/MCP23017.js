...
//	i2c1.writeByteSync(MCP23017_ADDRESS, IODIRB, 0x00); //Set all of bank B to outputs
	...
	i2c1.writeByteSync(MCP23017_ADDRESS, OLATB, 0x00); //Set all of bank B to outputs	
	
//	i2c1.write_byte_data(ADDR,IODIRA,0x00) # all port A GPIOs are output
//	i2c1.write_byte_data(ADDR,IODIRB,0xF0) # GPIOs 0-3 of B port are output	
...
//		var current_state = dec2bin( i2c1.readByteSync(MCP23017_ADDRESS, GPA),8) + dec2bin(i2c1.readByteSync(MCP23017_ADDRESS, GPB),8);
		var dec = i2c1.readByteSync(MCP23017_ADDRESS, GPB);
		console.log("readByteSync: ", dec);
		
		var current_state = dec2bin( dec, 8);
		...
		if(last_state !== current_state){
  			...
  			current_array.forEach(function(value, i) {
  				...
  				if(last_array[i] !== value){
  					...
  					if("0x20-GPB5".indexOf(code) >= 0){
//  						console.log("Lights Scan", last_state, current_state, code);
...
//  		if( duration > 1400 && duration < 2600){	
		  if( duration > 1400 && duration < 3500){
  			...
  			current_array.forEach(function(value, i) {
  				...
  				if(last_array[i] !== value){
  					var code = "0x20-GP" + ( (i<8) ? ("A"+i): ("B"+(i-8)));
  					if("0x20-GPB0|0x20-GPB1|0x20-GPB2|0x20-GPB3|0x20-GPB4|0x20-GPB5|0x20-GPA6|0x20-GPA7".indexOf(code) < 0){
...
//  			 console.log("DURATION2: ", duration, new Date());
//  			console.log(last_state, current_state);
//  		}
  		last_state = current_state;
  		lastTime = new Date();
    }
