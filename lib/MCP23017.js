...
try {
	...
	i2c1.writeByteSync(MCP23017_ADDRESS_0x20, IODIRA, 0x00); //Set all of bank A to outputs 
	i2c1.writeByteSync(MCP23017_ADDRESS_0x20, IODIRB, 0x00); //Set all of bank B to outputs	
...
module.exports.scanLights = function (callBack) {
	if(i2c1 === null){return;}
...
module.exports.scanPirSensors = function (callBack) {
	if(i2c1 === null){return;}
	...
	setInterval(function() {    
		...
		var current_state = dec2bin( i2c1.readByteSync(MCP23017_ADDRESS_0x20, GPIOA), 8).split("").reverse().join("") +  dec2bin( i2c1.readByteSync(MCP23017_ADDRESS_0x20, GPIOB), 8).split("").reverse().join("");
		
		i2c1.writeByteSync(MCP23017_ADDRESS_0x20, IODIRA, 0x00);
		i2c1.writeByteSync(MCP23017_ADDRESS_0x20, IODIRB, 0x00);
...
//		  console.log(duration, last_state, current_state, new Date());
//  		if( duration > 1400 && duration < 2600){	
		  if( duration > 1400 && duration < 3500){
  			var last_array = last_state.split('');
  			var current_array = current_state.split('');
  			current_array.forEach(function(value, i) {
  				if(last_array[i] !== value){
  					var code = "0x20-GP" + ( (i<8) ? ("A"+i): ("B"+(i-8)));
  					var state = last_array[i] + value;
  					var date = new Date();
  					console.log("MCP23017 ", code, state, date);
	  				callBack({code: code, state : state, date: date});
  				}
  		    });	
  		}
  		last_state = current_state;
