...
//
//ProgramsSchema.methods.toJSON = function() {
...
//----------- SCHEDULERS ---------------------------------------------
var SchedulersSchema = new Schema({
  code: String,
  name: String,
...
/**
 * commands : [{type:"light", id:"", action:"on/off"}]
 */

SchedulersSchema.methods.toJSON = function() {
	  var obj = this.toObject();
	  var date = moment(obj.date), formatted = date.format('DD/MM/YYYY HH:mm:ss');
	  obj.date = formatted;

	  var lastOccurrence = moment(obj.lastOccurrence);
	  obj.lastOccurrence = lastOccurrence.format('DD/MM/YYYY HH:mm:ss');

	  var nextOccurrence = moment(obj.nextOccurrence);
	  obj.nextOccurrence = nextOccurrence.format('DD/MM/YYYY HH:mm:ss');
	  
	  return obj;
};

module.exports.SCHEDULERS = mongoose.model('SCHEDULERS', SchedulersSchema);
...
var ConfigurationSchema = new Schema({
	...
	outsideSiren: {
					auto_off_timer: {type : Number, 'default': 10} //10 min == 60000 * 10 ms
				  }
});
...
var UispStandingSchema = new Schema({
	  team: String,
	  points: Number,
	  zone: String,
	  date: Date
	...
	module.exports.UISP_STANDING = mongoose.model('UISP_STANDING', UispStandingSchema);
